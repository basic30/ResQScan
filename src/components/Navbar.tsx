...
export default function Navbar() {
  const { isAuthenticated, logout, language, setLanguage, t } = useApp();
        ...
        <div className="flex items-center justify-between h-16 md:h-18">
          ...
          <Link to="/" className="flex items-center gap-2 group" onClick={() => setMobileOpen(false)}>
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400 -mt-1 hidden sm:block">{t('tagline')}</p>
            </div>
          </Link>
            ...
            <div className="flex items-center bg-white/5 rounded-xl p-1 mr-2">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  language === 'en' ? 'bg-gradient-to-r from-red-500 to-orange-500 text-white shadow-lg' : 'text-gray-400 hover:text-white'
                }`}
              >
                🇬🇧 EN
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  language === 'hi' ? 'bg-gradient-to-r from-red-500 to-orange-500 text-white shadow-lg' : 'text-gray-400 hover:text-white'
                }`}
              >
                🇮🇳 हि
              </button>
        ...
        {mobileOpen && (
          ...
          >
            <div className="px-4 py-4 space-y-2">
              {/* Language Toggle */}
              <div className="flex items-center bg-white/5 rounded-xl p-1 mb-3">
                <button
                  onClick={() => setLanguage('en')}
                  className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                    language === 'en' ? 'bg-gradient-to-r from-red-500 to-orange-500 text-white' : 'text-gray-400'
                  }`}
                >
                  🇬🇧 English
                </button>
                <button
                  onClick={() => setLanguage('hi')}
                  className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                    language === 'hi' ? 'bg-gradient-to-r from-red-500 to-orange-500 text-white' : 'text-gray-400'
                  }`}
                >
                  🇮🇳 हिंदी
                </button>
